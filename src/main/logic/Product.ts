...
export class Product {
  ...
  constructor(baseAddress: string, type: ProductType, sellPrice: number) {
    this.type = type
    this.sellPrice = sellPrice
    const key = `${baseAddress}-${ProductType[type]}`
    const id = productIds[key]
    if (!id) throw new Error(`Unknown product id mapping for ${key}`)
    this.id = id
  }
}

// NOTE: This is a direct port of the C# dictionary keys
